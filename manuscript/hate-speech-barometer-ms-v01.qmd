...
Given the societal impact of hate speech and the vibrating progress in ML, 
applied researchers are in desperate need of tools and templates to enable them to investigate social science research question.
...
Our intended audience are social scientists with an intermediate technical knowledge in statistics and ML.
...
The source code can be accessed freely online^[<https://github.com/sebastiansauer/hate-speech-barometer>].
Towards that end, we provide a typical ML pipeline is provided including well-known steps such as tuning different ML algorithms and employing resampling schemes.
In addition, we made use of GNU-Make-like project management tools for increased reproducibility and usability, see details below.
...
Our analysis is purely based on scripts which are freely accessible and permissively licenced (GNU General Public Licence).
Git was used as a versioning tool so that all changes to the code base can be made explicit.
A package management tool (renv; @ushey_renv_2023) is employed so that users can rest assured that the correct version of the R packages  are in place.
...
Some of the packages of the Tidyverse ecosystem, are among the most downloaded and most depended upon R packages^[<https://www.r-pkg.org/>, accessed 2023-05-25].
The Tidyverse authors state that the Tidyverse's "primary goal is to facilitate a conversation between a human and a computer about data" [p. 1; @wickham_welcome_2019]. 
One advantage of any (good) approach that is widely accepted is that it provides a standard for how things should get done.
Maybe one of the prime reason of the success of the Tidyverse is that it addressed key problems of data practitioners and finds sensible balances between conflicting goals. 
...
It has been said that perhaps the most difficult problem in computer science is naming objects^[<https://stackoverflow.com/questions/33497879/why-is-the-hardest-part-of-programming-is-naming-things>].
Then maybe the next most difficult thing is dealing with complexity, at least seen from a helicopter view. 
As an illustration of how complexity can creep in, consider the following example.
Given a set of 10 possible actions, where you have to pick the correct 3 to solve a problem leaves you with 120 possibilities (as combinatoric math demands). 
However, given a situation where you have to again pick 3 out of a set of 20, you are confronted you with an enormous 1120 possible combinations (3 out of 30: 4060). 
...
Just like Make, Targets take care that the objects of a pipeline are updated if and only if necessary. That means if an "upstream" objects changes and if this object is an input to a downstream object, then (and only then) the downstream object will  be updated.
